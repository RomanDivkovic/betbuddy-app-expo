...
// Group Services
export const createGroup = async (groupData: Omit<Group, 'id' | 'adminId' | 'members' | 'createdAt'>) => {
...
export const savePrediction = async (
  ...
  const predictionRef = ref(db, `groups/${groupId}/predictions/${eventId}/${user.uid}/${matchId}`);
  await set(predictionRef, prediction);

  return prediction;
};

export const fetchUserPredictions = async (groupId: string, eventId: string) => {
  const user = auth.currentUser;
  if (!user) return [];

  try {
    const predictionsRef = ref(db, `groups/${groupId}/predictions/${eventId}/${user.uid}`);
...
export const getEventLeaderboard = async (groupId: string, eventId: string): Promise<LeaderboardEntry[]> => {
  try {
    ...
    const [leaderboardSnapshot, predictionsSnapshot, membersSnapshot] = await Promise.all([
      get(leaderboardRef),
      get(predictionsRef),
      get(membersRef)
    ]);
    ...
    const predictions = predictionsSnapshot.exists() ? predictionsSnapshot.val() : {};
